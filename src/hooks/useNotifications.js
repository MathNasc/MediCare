 ...
 *
 * Toda a lógica de "agendar" lembretes saiu do cliente. Quem decide quando
 * notificar é a Edge Function `send-medication-reminders`, disparada a cada
 * minuto via pg_cron no servidor (Supabase). Isso é necessário porque
 * Service Workers são encerrados pelo navegador quando ociosos — o
 * `setInterval` que existia antes dentro do SW parava de rodar assim que o
 * app saía de primeiro plano, então nunca era confiável para avisar o
 * paciente com o app fechado. Um push real vindo do servidor acorda o SW
 * mesmo dormente, que é como esse mecanismo foi desenhado para funcionar.
 *
